...
class WebLogGeneration(object):
    def __init__(self):
        ...
        self.ip_slice_list = [10, 29, 30, 466, 55, 63, 72, 87, 98, 132, 156, 124, 167, 143, 187, 168, 190, 201, 202,
                              214, 215, 222]
    ...
    def sample_user_agent(self):
        ...
        return random.sample(self.user_agent_dist, 1)[0]

    def sample_refer(self):
        if random.uniform(0, 1) > 0.2:
            return "-"
        refer_str = random.sample(self.http_refer, 1)
        qquery_str = random.sample(self.search_keyword, 1)
        return refer_str[0].format(query=qquery_str[0])

    def sample_one_log(self, count=3):
        time_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        while count > 1:
            qery_log = "{ip} - - [{local_time}] \"GET /{url} HTTP/1.1\" 200 0 \"{refer}\" \"{user_agent}\" \"-\"" \
                .format(
